...
class ModelManager:
    ...
    def _load_predictions(self) -> None:
        ...
        try:
            ...
            logger.info(f"Raw date column values: {predictions_df['date'].tolist()}")
            ...
            if predictions_df['date'].isnull().any():
                raise ValueError("Invalid date format in predictions.csv after coercion")
            
            # Validate predicted_demand values
            predictions_df['predicted_demand'] = pd.to_numeric(predictions_df['predicted_demand'], errors='coerce')
            if predictions_df['predicted_demand'].isnull().any():
                raise ValueError("Invalid predicted_demand values in predictions.csv")
            ...
            for model in model_names:
                ...
                for _, row in model_preds.iterrows():
                    date = row['date']
                    if pd.isna(date):
                        raise ValueError(f"Encountered invalid date for model {model}")
                    formatted_date = date.strftime('%B %Y')
                    model_dict[formatted_date] = float(row['predicted_demand'])
            ...
            logger.info("Loaded predictions successfully")
        ...
        try:
            ...
            for name, model in self.models.items():
                try:
                    ...
                    if len(y_pred) < len(target_dates):
                        raise ValueError(f"Model {name} predicted {len(y_pred)} values, expected {len(target_dates)}")
            ...
            # Reload predictions to ensure consistency
            self._load_predictions()
